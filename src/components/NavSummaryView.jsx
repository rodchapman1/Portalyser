...
function NavStackedBar({ parts, nav }) {
    const positives = parts.filter((part) => part.value > 0);
    const negatives = parts.filter((part) => part.value < 0);
    const scale = Math.max(Math.abs(nav || 0), positives.reduce((sum, part) => sum + part.value, 0), 1);
    return (
        <div className="mt-4">
            <div className="flex h-6 overflow-hidden rounded-md bg-gray-800">
                {positives.length === 0 ? (
                    ...
                    <div
                        key={part.key}
                        className="h-full"
                        style={{ width: `${(part.value / scale) * 100}%`, background: part.color }}
            ...
            {negatives.length > 0 && (
                <p className="mt-2 text-xs text-gray-400">
                    Negative NAV parts (reduce the total):{' '}
                    {negatives.map((part) => `${part.name} ${formatMoneyExact(part.value)}`).join(', ')}
                </p>
            )}
        </div>
    );
...
function NavCard({ title, book, keys, emptyHint }) {
    ...
    return (
        <section className="rounded-2xl border border-gray-700 bg-gray-900 p-5">
            ...
            <NavStackedBar parts={parts} nav={nav?.nav || 0} />
                        ...
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <div className="text-xs text-gray-500">
                                    {SOURCE_LABELS[part.source] || part.source || 'Unavailable — 0'}
                                </div>
                            </div>
                        </div>
